...
class MarketingFirm:
    ...
    def create_sweepstakes(self):
        new_sweepstakes = User_Interface.get_user_input_string("Please enter the name for the new sweepstakes.")
        self.sweepstakes_storage.append(new_sweepstakes)

    def change_marketing_firm_name(self):
        new_firm_name = User_Interface.get_user_input_string("Please enter the name for the new marketing firm.")
    ...
    def menu(self):
        display_marketing_firm_menu_options(self.name)
        user_input = get_user_input_number("Please Choose from the menu")
        if user_input == 0:
            sweepstakes_selection_menu(self.sweepstakes_storage)
        if user_input == 1:
